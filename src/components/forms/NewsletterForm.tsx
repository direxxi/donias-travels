...
export default function NewsletterForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setStatus("loading");
    try {
      await submitForm({ formType: "newsletter", email, source });
      setStatus("success");
    } catch {
      setStatus("error");
    }
  ...
  if (status === "success")
    return (
      <div className="flex items-center justify-center gap-3 py-3">
        ...
        <span
          className={`font-medium ${dark ? "text-white" : "text-secondary"}`}
        >
    ...
    >
      <input
        ...
        className={`flex-1 px-4 py-3 rounded-lg text-sm focus:outline-none focus:ring-2 ${
          dark
            ? "bg-white/10 border border-white/20 text-white placeholder-white/50 focus:ring-white/40"
            : "bg-white border border-brand-border text-secondary placeholder-secondary/40 focus:ring-primary/30 focus:border-primary"
        }`}
      />
      <button
        type="submit"
